...
namespace DroneDelivery
{
    ...
    {
        public static string[] ApplicationRoles = new string[] { "Admin", "User", "Customer" };
        ...
        {
            var roleManager = new ApplicationRoleManager(new RoleStore<IdentityRole>(context.Get<DatabaseContext>()));
            ...
            foreach (var role in ApplicationRoles)
            {
                if (!roleManager.RoleExists(role))
                {
                    ...
                    if (!result.Succeeded)
                        throw new ApplicationException("Creating role: " + role + " failed. Error: " + result.Errors);
                }
            }
